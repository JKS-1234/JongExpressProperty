 ...
 * build.js
 *
 * Fetches the live property CSV (Google Sheets) at BUILD TIME and generates:
 *   - /data/properties.json          used by js/script.js for client-side rendering/filtering
...
function buildPropertyPageHTML(p) {
  ...
  const iconsHTML =
    beds || baths || parking
      ? `
        <div class="icon-row">
            ${beds ? `<span>\u{1F6CF}\u{FE0F} ${esc(beds)}</span>` : ''}
            ${baths ? `<span>\u{1F6C1} ${esc(baths)}</span>` : ''}
            ${parking ? `<span>\u{1F697} ${esc(parking)}</span>` : ''}
        </div>`
      : '';

  const excludeColumns = [
    'Property Name', 'Price', 'Image Name', 'Video Link', 'The Good (Pros)',
    'Area', 'Timestamp', 'Bedrooms', 'Bathrooms', 'Car Park', 'originalId', 'slug',
  ];
  const pipeItems = Object.keys(p)
    .filter((k) => !excludeColumns.includes(k) && String(p[k] || '').trim() !== '')
    .map((k) => `<span>${esc(String(p[k]).trim())}</span>`)
    .join('');
  ...
  const sliderHTML = `
        <div class="image-slider-container">
            <div class="image-slider" id="pdp-slider">
                ${sliderImages
                  .map(
                    (img, i) => `<img src="${esc(img)}" alt="${esc(title)} photo ${i + 1}" loading="${i === 0 ? 'eager' : 'lazy'}" class="slider-img" onclick="openLightbox('pdp-slider', ${i})" onerror="this.src='https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=800&q=80'">`
                  )
                  .join('')}
            </div>
            ${
              sliderImages.length > 1
                ? `
            <button class="slider-btn slider-btn-prev" onclick="slideImage('pdp-slider', -1)">&#10094;</button>
            <button class="slider-btn slider-btn-next" onclick="slideImage('pdp-slider', 1)">&#10095;</button>`
                : ''
            }
        </div>`;

  const videoHTML = ytEmbed
    ? `<div class="video-container"><iframe src="${ytEmbed}" allowfullscreen loading="lazy"></iframe></div>`
    : videoLink
    ? `<a href="${esc(videoLink)}" class="video-btn" target="_blank" rel="noopener noreferrer">\u{1F3AC} Watch Video Tour</a>`
    : '';
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Listings', item: `${SITE_URL}/listing.html` },
...
<body>
    ...
    <a href="https://wa.me/60169242000?text=Hi%20Jong,%20I%20am%20interested%20in%20${encodeURIComponent(title)}" target="_blank" rel="noopener noreferrer" style="position: fixed; bottom: 20px; left: 20px; background-color: #25D366; color: white; border-radius: 50px; padding: 12px 20px; font-size: 16px; text-decoration: none; font-weight: bold; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); z-index: 1000;">
        \u{1F4AC} WhatsApp Us
    </a>

    <script>
      window.lightboxGalleries = { "pdp-slider": ${JSON.stringify(sliderImages)} };
...
async function main() {
  ...
  // 2. One static, indexable page per property
  ...
  console.log(`Generated ${rows.length} property pages in /properties/`);
