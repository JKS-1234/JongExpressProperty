...
Papa.parse(csvUrl, {
    ...
    complete: function(results) {
        ...
        allPropertiesData.forEach(row => {
            if (row['Type']) uniqueTypes.add(row['Type'].trim());
            if (row['Area']) uniqueAreas.add(row['Area'].trim());
        });
...
function filterProperties() {
    ...
    if (filteredData.length === 0) {
        ...
        filteredData.slice(0, currentLimit).forEach((row) => {
            ...
            let iconsHTML = '';
            if (beds || baths) {
                iconsHTML = `<div class="icon-row"><span style="display:flex; align-items:center; gap:5px;">🛏️ ${beds}</span><span style="display:flex; align-items:center; gap:5px;">🛁 ${baths}</span></div>`;
            }

            let rawImages = row['Image Name'] ? String(row['Image Name']).trim() : '';
            let firstImage = rawImages.split(',').map(url => url.trim()).filter(url => url !== '')[0] || 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=600&q=80';

            allCardsHTML += `
            <div class="property-card clickable-card" style="display:flex; flex-direction:column; height:100%;">
                ${badgeHTML}
                <div onclick="openModal(${globalIndex})" style="cursor:pointer;">
...
function openModal(index) {
    ...
    let beds = row['Bedrooms'] || '-';
    let baths = row['Bathrooms'] || '-';
    ...
    document.getElementById('modal-main-img').src = imagesArr.length > 0 ? imagesArr[0] : 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=600&q=80';
